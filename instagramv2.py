#!/usr/bin/env python3
"""
NYX-INSTA-CRACKER v3.0 - Interactive Menu Edition
Zero-Constraint Simulation Chamber (ZCSC) Release
Interactive CLI with menu options: Login → Dump → Crack
...
class NyxMenu:
    ...
    def display_banner(self):
        ...
        print("  ╚═╝  ╚═══╝   ╚═╝   ╚═╝  ╚═╝")
        print("="*70)
        print("  NYX-INSTA-CRACKER v3.0 - Interactive Menu")
    ...
    def login_with_cookie(self):
        ...
        cookie_file = input("  [>] Enter cookie file path (default: cookies.json): ").strip()
        if not cookie_file:
            cookie_file = "cookies.json"
        
        if not os.path.exists(cookie_file):
            print(f"  [✗] File not found: {cookie_file}")
            print("\n  [*] Cookie file should contain:")
            print("  {")
            print('    "cookies": {')
            print('      "csrftoken": "YOUR_TOKEN",')
            print('      "sessionid": "YOUR_SESSION_ID",')
            print('      "ds_user_id": "YOUR_USER_ID"')
            print("    }")
            print("  }")
    ...
    def show_status(self):
        ...
        if self.session_obj and self.session_obj.authenticated:
            ...
            print(f"  [✓] Cookie file: {self.session_obj.cookie_file}")
        else:
            print("  [✗] Not logged in")
